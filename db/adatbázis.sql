...
(
    ...
    CONSTRAINT `fk_anime_elozmeny`
        FOREIGN KEY (`elozmeny_id`) REFERENCES `hazi`.`anime` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
    CONSTRAINT `fk_anime_folytatas`
        FOREIGN KEY (`folytatas_id`) REFERENCES `hazi`.`anime` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
    CONSTRAINT `fk_anime_szezon`
        FOREIGN KEY (`szezon_id`) REFERENCES `hazi`.`szezon` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8;
...
(
    ...
    CONSTRAINT `fk_felhasznalo_has_anime_felhasznalo1`
        FOREIGN KEY (`felhasznalo_id`) REFERENCES `hazi`.`felhasznalo` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
    CONSTRAINT `fk_felhasznalo_has_anime_anime1`
        FOREIGN KEY (`anime_id`) REFERENCES `hazi`.`anime` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8;
...
(
    ...
    CONSTRAINT `fk_anime_has_studio_studio1`
        FOREIGN KEY (`studio_id`) REFERENCES `hazi`.`studio` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE = InnoDB
  DEFAULT CHARSET = utf8;
...
(
    ...
    CONSTRAINT `fk_anime_has_karakter_karakter1`
        FOREIGN KEY (`karakter_id`) REFERENCES `hazi`.`karakter` (`id`) ON DELETE NO ACTION ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `hazi`.`kedvenc_szinész`
(
    `felhasznalo_id`      INT(11) NOT NULL,
    `szinkron_szinész_id` INT(11) NOT NULL,
